...
GoombaObject.prototype.draw = function (spriteList) {
    if (!this.disabled) {
        var mode;
        mode = this.state === GoombaObject.STATE.BONK ? 0x3 : 0x0 < this.disabledTimer ? 0xa0 + parseInt(0x20 * (0x1 - this.disabledTimer / GoombaObject.ENABLE_FADE_TIME)) : 0x0;
        if (this.sprite.INDEX instanceof Array)
            for (var _0x2d049f = this.sprite.INDEX, _0x2e81c8 = 0x0; _0x2e81c8 < _0x2d049f.length; _0x2e81c8++)
                for (var _0x28feb8 = 0x0; _0x28feb8 < _0x2d049f[_0x2e81c8].length; _0x28feb8++) {
                    var index = _0x2d049f[mode ? _0x2d049f.length - 0x1 - _0x2e81c8 : _0x2e81c8][_0x28feb8];
                    switch (this.variant) {
                        case 0x1:
        ...
        if (this.sprite.INDEX instanceof Array)
            for (var _0x5bdb01 = this.sprite.INDEX, _0x98a94 = 0x0; _0x98a94 < _0x5bdb01.length; _0x98a94++)
                for (var _0x3f41db = 0x0; _0x3f41db < _0x5bdb01[_0x98a94].length; _0x3f41db++) {
                    var index = _0x5bdb01[0x3 !== mode ? _0x98a94 : _0x5bdb01.length - 0x1 - _0x98a94][_0x3f41db];
                    switch (this.variant) {
                        case 0x1:
...
for (var i = 0x0; i < HammerBroObject.SPRITE_LIST.length; i++) HammerBroObject.SPRITE[HammerBroObject.SPRITE_LIST[i].NAME] = HammerBroObject.SPRITE_LIST[i], HammerBroObject.SPRITE[HammerBroObject.SPRITE_LIST[i].ID] = HammerBroObject.SPRITE_LIST[i];
...
for (var i = 0x0; i < HammerBroObject.STATE_LIST.length; i++) HammerBroObject.STATE[HammerBroObject.STATE_LIST[i].NAME] = HammerBroObject.STATE_LIST[i], HammerBroObject.STATE[HammerBroObject.STATE_LIST[i].ID] = HammerBroObject.STATE_LIST[i];
...
BowserObject.prototype.draw = function (spriteList) {
    var mode;
    mode = this.state === BowserObject.STATE.BONK ? 0x3 : 0x0;
    if (this.sprite.INDEX instanceof Array)
        for (var _0x132286 = this.sprite.INDEX, _0x1479b4 = 0x0; _0x1479b4 < _0x132286.length; _0x1479b4++)
            for (var j = 0x0; j < _0x132286[_0x1479b4].length; j++) spriteList.push({
                'pos': vec2.add(this.pos, vec2.make(j, _0x1479b4)),
                'reverse': !this.dir,
                'index': _0x132286[mode ? _0x132286.length - 0x1 - _0x1479b4 : _0x1479b4][j],
                'mode': mode
            });
...
SpringObject.prototype.draw = function (spriteList) {
    if (this.sprite.INDEX instanceof Array)
        for (var _0x3191ef = this.sprite.INDEX, _0x4714a0 = 0x0; _0x4714a0 < _0x3191ef.length; _0x4714a0++)
            for (var _0x507d31 = 0x0; _0x507d31 < _0x3191ef[_0x4714a0].length; _0x507d31++) spriteList.push({
...
function FlagpoleObject(game, level, zone, pos, oid, noOffset) {
    GameObject.call(this, game, level, zone, pos);
    this.oid = oid;
    this.setState(FlagpoleObject.STATE.IDLE);
    this.anim = 0x0;
    this.offset = parseInt(noOffset) === 1 ? vec2.make(0x0, 0x0) : FlagpoleObject.OFFSET;
}
...
for (var i = 0x0; i < BillBlasterObject.SPRITE_LIST.length; i++) BillBlasterObject.SPRITE[BillBlasterObject.SPRITE_LIST[i].NAME] = BillBlasterObject.SPRITE_LIST[i], BillBlasterObject.SPRITE[BillBlasterObject.SPRITE_LIST[i].ID] = BillBlasterObject.SPRITE_LIST[i];
...
for (var i = 0x0; i < BillBlasterObject.STATE_LIST.length; i++) BillBlasterObject.STATE[BillBlasterObject.STATE_LIST[i].NAME] = BillBlasterObject.STATE_LIST[i], BillBlasterObject.STATE[BillBlasterObject.STATE_LIST[i].ID] = BillBlasterObject.STATE_LIST[i];
...
for (var i = 0x0; i < BulletBillObject.SPRITE_LIST.length; i++) BulletBillObject.SPRITE[BulletBillObject.SPRITE_LIST[i].NAME] = BulletBillObject.SPRITE_LIST[i], BulletBillObject.SPRITE[BulletBillObject.SPRITE_LIST[i].ID] = BulletBillObject.SPRITE_LIST[i];
...
for (var i = 0x0; i < BulletBillObject.STATE_LIST.length; i++) BulletBillObject.STATE[BulletBillObject.STATE_LIST[i].NAME] = BulletBillObject.STATE_LIST[i], BulletBillObject.STATE[BulletBillObject.STATE_LIST[i].ID] = BulletBillObject.STATE_LIST[i];
...
BulletBillObject.prototype.draw = function (spriteList) {
    var mode;
    mode = this.state === BulletBillObject.STATE.BONK ? 0x3 : 0x0;
    spriteList.push({
...
td32.state = function (_0x4f1547) {
    ...
    const gnipmujCheck = 0x14 < collideTestResult[td32.collideTest("gnipmuj")];
...
function Game(data) {
    ...
    this.lives = 0x3;
    if(app.net.gameMode === 1) { this.lives = 0x0; }
...
Game.prototype.doStep = function () {
    ...
    if(player && !player.dead && !this.pauseCamera) {
        switch (zone.camera) {
            case 0 : { this.display.camera.position(vec2.make(this.overrideCameraPos ? this.overrideCameraPos.x : player.pos.x, 0.5 * zone.dimensions().y)); break; } // Horizontal Scrolling
            case 1 : { this.display.camera.positionX(zone.dimensions().x*.5); this.display.camera.positionY(Math.min(zone.dimensions().y-7, -player.pos.y + zone.dimensions().y)); break; } // Vertical Scrolling (horizontal is always centered)
            case 2 : {
                ...
                this.display.camera.positionY(Math.min(zone.dimensions().y-7, -player.pos.y + zone.dimensions().y));
                break;
            }
        }
    }
...
Game.prototype.doMusic = function () {
    ...
    if (this.gameOver) {
        ...
        zone.victorymusic !== '' ? (app.audio.setMusic(zone.victorymusic), this.victoryMusic = true) : /* Skip castle.mp3 if victorymusic is present */ app.audio.setMusic(zone.winmusic || "castle.mp3", false);
...
App.prototype.join = function (name, team, priv, skin, gm, badges) {
    ...
    this.ingame() ? this.menu.error.show("An error occured while starting game...") : (this.menu.load.show(), this.net.connect([Network.CONNECTTYPE.GUEST, name, team, priv, skin, gm, badges]));
};
