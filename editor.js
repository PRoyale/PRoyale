...
var tileDefs = {
    ...
    14: {'name': 'FLIP BLOCK', 'extraDataName': 'Animation ID'},
    ...
    27: {'name': 'HALF TILE BUMPABLE'},
    28: {'name': 'HALF TILE SOLID'},
    29: {'name': 'HALF TILE SEMISOLID'},
    30: {'name': 'LOCK X AXIS CAMERA'},
    31: {'name': 'UNLOCK X AXIS CAMERA'},
    32: {'name': 'LOCK Y AXIS CAMERA'},
    33: {'name': 'UNLOCK Y AXIS CAMERA'},
    // reserve 34 and 35 for lock & unlock both axis
    36: {'name': 'PLAYER BARRIER'},
    37: {'name': 'ENEMY BARRIER'},
    38: {'name': 'SOLID 1:1 SLOPE'},
...
var objDefs = {
    1:   {
        'name': 'PLAYER',
        'description': '- Creates a dummy player that can work like a regular player\n- [skin] Positive integer ID from available skins\n - [isDev,isMod] Serves no purpose, but should be 0 or 1.\n- [followPlayer] CLIENT SIDE! Follows you in game.\n- [nameTag] This text will appear above this object as a name\n- [killTarget] If a player collides with this then that player will die. Intended to be used with followPlayer',
        'paramDefs': [
            {'name': 'skin','type': 'int'},
            {'name': 'isDev', 'type': 'bool'},
            {'name': 'isJunior', 'type': 'bool'},
            {'name': 'isMod', 'type': 'bool'},
            {'name': 'followPlayer', 'type': 'bool'},
            {'name': 'nameTag', 'type': 'string'},
            {'name': 'killTarget', 'type': 'bool'}
        ]
    },
    16:  {'name': 'GOOMBRAT', 'description': '- No params.'},
    17:  {'name': 'GOOMBA', 'description': '- [variant] Should be set to 0, 1, or 2. 0 is the light/overworld one, 1 is the dark/underground one, 2 is the gray/castle one.', 'paramDefs': [{'name': 'variant','type':'int'}]},
    18:  {'name': 'KOOPA TROOPA GREEN', 'description': '- [fly] Should be set to 0 or 1. 1 is flying.\n- [variant] Should be set to 0, 1, or 2. 0 is the light/overworld one, 1 is the dark/underground one, 2 is the gray/underwater one.', 'paramDefs': [{'name': 'fly','type':'bool'}, {'name': 'variant','type':'int'}]},
    ...
    20:  {'name': 'KOOPA SHELL', 'description': 'Koopa shell. Cannot turn back into a koopa.\n- [variant] 0 is light variant, 1 is dark.', 'paramDefs': [{'name': 'variant', 'type': 'bool'}]},
    ...
    22:  {'name': 'PIRANHA PLANT', 'description': '- [variant] Should be set to 0, 1, or 2. 0 is the light one, 1 is the dark one, 2 is the red one.\n- [direction] Indicates which way to go. 0 is normal, 1 is upside down.\n- [static] Should be 0 or 1. 0 is normal, 1 is static.\n- [noOffset] By default Piranha Plants are offset by half a tile to fit in two tiled pipes. Use 1 to stop that from happening.', 'paramDefs': [{'name': 'variant','type':'int'}, {'name': 'direction','type':'bool'}, {'name': 'static','type':'bool'}, {'name': 'noOffset','type':'bool'}]},
    23:  {'name': 'SPINY', 'description': '- [variant] Should be set to 0 or 1. 0 is the normal spiny, 1 is a Metal Spiny that’s immune to fireballs.', 'paramDefs': [{'name': 'variant','type':'bool'}]},
    24:  {'name': 'BUZZY BEETLE', 'description': '- [variant] Should be set to 0, 1, or 2. 0 is the light/overworld one, 1 is the dark/underground one, 2 is the gray/castle one.', 'paramDefs': [{'name': 'variant','type':'int'}]},
    25:  {'name': 'BOWSER', 'description': '- [attackType] Should be set to 0, 1, 2, or -1. 0 is only fire breathing, 1 is only hammer throwing, 2 is both, -1 is neither.\n- [fireDirection] Should be set to 0 or 1. 0 is left, 1 is right.\n- [variant] Should be set to 0 or 1. 0 is the normal green one, 1 is the blue one (Bowser’s Brother).', 'paramDefs': [{'name': 'attackType','type':'int'}, {'name': 'fireDirection','type':'int'}, {'name':'variant', 'type':'int'}]},
    26:  {'name': 'DRY BONES', 'description': '- [fly] Should be set to 0 or 1. 1 is flying.', 'paramDefs': [{'name': 'fly','type':'bool'}]},
    31:  {
        'name': 'BOO',
        'description': '- [variant] Should be set to 0 or 1. 0 is the light one, 1 is the dark one.',
        'paramDefs': [
            {'name': 'variant', 'type': 'bool'}
        ]
    },
    32:  {'name': 'ROTO DISC', 'description': '- [spacing] Distance from origin point\n- [rate] Rate of rotation. Same logic as firebars, lower is faster, 18 is default.', 'paramDefs': [{'name': 'spacing'}, {'name': 'rate'}]},
    ...
    35:  {'name': 'BILL BLASTER', 'description': '- [delay] Positive integer in seconds for how long it takes for it to shoot.\n- [direction] Indicates which way to go. 0 is left, 1 is right.\n- [speed] Speed of the bullet. Default is 0.215.\n- [life] How many frames (formula: x*30 [x=seconds]) until the bullet self destructs. Leave blank if you want the bullet to live forever\n- [variant] Should be set to 0, 1, or 2. 0 is the light/overworld one, 1 is the dark/underground one, 2 is the gray/castle one.', 'paramDefs': [{'name': 'delay', 'type': 'unsigned int'}, {'name': 'direction', 'type': 'bool'}, {'name': 'speed', 'type': 'float'}, {'name': 'life', 'type': 'int'}, {'name': 'variant', 'type': 'int'}]},
    36:  {'name': 'BULLET BILL', 'description': '- [direction] Direction of the bullet. 0 is left, 1 is right.\n- [speed] Speed of the bullet, default is 0.215.\n- [life] How many frames (formula: x*30 [x=seconds]) until the bullet self destructs. Leave blank if you want the bullet to live forever\n- [variant] Should be set to 0, 1, or 2. 0 is the light/overworld one, 1 is the dark/underground one, 2 is the gray/castle one.\n- It is better to use the BILL BLASTER object.', 'paramDefs': [{'name': 'direction', 'type': 'bool'}, {'name': 'speed', 'type': 'float'}, {'name': 'life', 'type': 'int'}, {'name': 'variant', 'type': 'int'}]},
    ...
    38:  {'name': 'BANZAI BLASTER', 'description': '- Shoots a banzai bill.\n- [delay] Positive integer in seconds for how long it takes for it to shoot.\n- [direction] Indicates which way to go. 0 is left, 1 is right.\n- [speed] Speed of the bullet. Default is 0.215.', 'paramDefs': [{'name': 'delay', 'type': 'unsigned int'}, {'name': 'direction', 'type': 'bool'}, {'name': 'speed', 'type': 'float'}]},
    39:  {'name': 'BANZAI BILL', 'description': '- [direction] Direction of the bullet. 0 is left, 1 is right.\n- [speed] Speed of the bullet, default is 0.215.\n- If you want the Banzai Bill to shoot multiple times, consider the BANZAI BLASTER object', 'paramDefs': [{'name': 'direction'}, {'name': 'speed'}]},
    40:  {'name': 'REX', 'description': '- [variant] Should be set to 0 or 1. 0 is the light one, 1 is the dark one.', 'paramDefs': [{'name': 'variant','type':'bool'}]},
    41:  {'name': 'CHEEP CHEEP', 'description': '- As opposed to the flying fish, this one swims happily horizontally\n- [variant] Sets the color and speed of the fish. 0 (default) is red and swims faster. 1 is gray/green and swims slower. \n- [patrol] 0 is the normal fish that moves right to left. 1 is the patrolling cheep cheep.', 'paramDefs': [{'name': 'variant', 'type': 'bool'}, {'name': 'patrol', 'type': 'bool'}]},
    42:  {'name': 'THWOMP', 'description': '- It is a thwomp.'},
    49:  {'name': 'HAMMER BRO', 'description': '- [phase] Should be 0 or 1, indicates which way to go.\n- [variant] Should be set to 0 or 1. 0 is the light one, 1 is the dark one.', 'paramDefs': [{'name': 'phase', 'type': 'bool'}, {'name': 'variant', 'type': 'int'}]},
    50:  {'name': 'FIRE BRO', 'description': '- [phase] Should be 0 or 1, indicates which way to go.', 'paramDefs': [{'name': 'phase', 'type': 'bool'}]},
    81:  {'name': 'MUSHROOM', 'description': '- No params.'},
    82:  {'name': 'FIRE FLOWER', 'description': '- No params.'},
    83:  {'name': 'ONEUP', 'description': '- No params.'},
    84:  {'name': 'STAR', 'description': '- [static] Should be 0 or 1. 0 is normal, 1 is static.\n- Falling despite static being 1 is intentional.', 'paramDefs': [{'name': 'static'}]},
    85:  {'name': 'AXE', 'description': '- No params.'},
    86:  {'name': 'POISON MUSHROOM', 'description': '- No params.\n- Damages the player'},
    87:  {'name': 'CHECKPOINT', 'description': '- [powerup] Powerup to give. Corresponds to: (0,1,2,3,4,5)(nothing,mushroom,fireflower,star,oneup,poison)', 'paramDefs': [{'name': 'powerup', 'type': 'int'}]},
    97:  {'name': 'COIN', 'description': '- No params.'},
    100: {'name': 'GOLD FLOWER', 'description': '- No params.'},
    145: {'name': 'PLATFORM', 'description': '- [length] Integer which decides how many tiles the platform should be.\n- [offX] Float integer, how far to move on X axis\n- [offY] Float integer, how far to move on Y axis\n- [speed] Float, moving speed of the platform\n- [loop] 0 or 1. If it is 1, the platform will immediately return to its starting position once done.\n- [delay] Positive int, inital delay on platform. Used to offset 2 platforms moving on same path\n- [direction] 0 or 1, starts at end point', 'paramDefs': [{'name': 'length'}, {'name': 'offX'}, {'name': 'offY'}, {'name': 'speed'}, {'name': 'loop'}, {'name': 'delay'}, {'name': 'direction'}]},
    ...
    147: {'name': 'PATH-FOLLOWING PLATFORM', 'description': '- Follows a given path. Format: [a,b][c,d][e,f] etc.\n- [length] Length of the platform in tiles. Anchor is left!\n- [speed] Speed of the platform.\n- [delay] Initial delay when spawning platforms. Used to offset 2 platforms moving on the same path\n- [locations] Set of paths the platform should follow. Format: [a,b][c,d][e,f] etc', 'paramDefs': [{'name': 'length'}, {'name': 'speed'}, {'name': 'delay'}, {'name': 'locations'}]},
    149: {'name': 'SPRING', 'description': '- [springType] Should be 0 or 1. Use 0 for normal springs (which are red on the default object sheet), and 1 for super springs (green by default).', 'paramDefs': [{'name': 'springType', 'type': 'bool'}]},
    161: {'name': 'FIREBALL PROJECTILE', 'description': '- This object will not work properly.', 'paramDefs': [{'name': 'dir'},{'name': 'owner'},{'name': 'skin'}]},
    162: {'name': 'FIRE BREATH PROJECTILE', 'description': '- This object will not work properly.', 'paramDefs': [{'name': 'direction'}]},
    163: {'name': 'HAMMER PROJECTILE', 'description': '- This object will not work properly.', 'paramDefs': [{'name': 'owner'}, {'name': 'delay'}]},
    177: {'name': 'FLAG', 'description': '- Used for flagpoles so the flag goes down along with the player.\n- [noOffset] If set to 1, it won\'t use the default offset of -0.5, which is normally used for flagpoles\n- [variant] Should be set to 0 or 1. 0 is the light one, 1 is the dark one.', 'paramDefs': [{'name':'noOffset'},{'name':'variant'}]},
    178: {'name': 'GOAL POST', 'description': 'Goal post from Super Mario World.\n- [movy] How many tiles the goal post moves upwards', 'paramDefs': [{'name': 'movy', 'type': 'int'}]},
...
util.sprite.getSprite = function(sheet, index) {
    var width = sheet.width;
    sheet = sheet.height;
    index *= Display.TEXRES;
    var sprite = parseInt(Math.floor(index / width) * Display.TEXRES);
    return sprite > sheet ? [0x0, 0x0] : [index % width, sprite];
};
...
var shor2 = {},
    td32 = {};
    tdAny = {}; // Deluxe format
td32.encode = function(index, bump, depth, definition, data) {
    return 0x0 | parseInt(index) & 0x7ff | parseInt(bump) << 0xb & 0x7800 | (depth ? 0x1 : 0x0) << 0xf & 0x8000 | parseInt(definition) << 0x10 & 0xff0000 | parseInt(data) << 0x18 & 0xff000000;
};
...
td32.decode = function(a) {
    var definition = a >> 0x10 & 0xff;
    return {
        index: a & 0x7ff,
        bump: a >> 0xb & 0xf,
        depth: 0x1 === (a >> 0xf & 0x1),
        definition: td32.TILE_PROPERTIES[definition] ? td32.TILE_PROPERTIES[definition] : td32.TILE_PROPERTIES[0x0],
        data: a >> 0x18 & 0xff
    };
};
...
td32.asArray = function(a) {
    return [a & 0x7ff, a >> 0xb & 0xf, 0x1 === (a >> 0xf & 0x1), a >> 0x10 & 0xff, a >> 0x18 & 0xff];
};
td32.TRIGGER = {};
td32.GEN_FUNC = {};
td32.TRIGGER.TYPE = {};
...
shor2.encode = function(a, b) {
    return 0x0 | parseInt(a) & 0xffff | parseInt(b) << 0x10 & 0xffff0000;
};
shor2.decode = function(a) {
    return vec2.make(a & 0xffff, a >> 0x10 & 0xffff);
};
shor2.asArray = function(a) {
    return [a & 0xffff, a >> 0x10 & 0xffff];
};
...
function _0x3f2a38() {
    ...
    this.btnSettings = document.getElementById("editor-top-settings");
    this.btnAccessibility = document.getElementById("editor-top-accessibility");
    ...
    this.btnSettings.onclick = function() {
        app.menu.tool.set("settings");
    };
    this.btnSave.onclick = function() {
        if (app.editor.tool) app.editor.tool.save();
        app.save();
    };
    this.btnAccessibility.onclick = function() {
        var accessibilityCss = document.getElementById('accessibility-css');
        
        if (accessibilityCss) {
            accessibilityCss.parentNode.removeChild(accessibilityCss);
        } else {
            var link = document.createElement('link');
            link.rel = 'stylesheet';
            link.type = 'text/css';
            link.href = 'css/editor-accessibility.css';
    ...
    if (app.editor) {
        ...
        for (var i = 0x0; i < world.levels.length; i++) {
            ...
            for (var j = 0x0; j < level.zones.length; j++) {
                ...
                var itemId = "list-gen-" + level.id + '-' + zone.id;
        ...
        for (var i = 0x0; i < world.levels.length; i++) {
            ...
            for (var j = 0x0; j < level.zones.length; j++) {
                var zone = level.zones[j],
                    itemId = "list-gen-" + level.id + '-' + zone.id,
...
function WorldTool(editor) {
    ...
    this.valLongname = document.getElementById("editor-tool-world-longname");
    ...
    this.valTilesetPath = document.getElementById("editor-tool-world-tileset");
    this.valObjsheetPath = document.getElementById("editor-tool-world-objsheet");
...
WorldTool.prototype.load = function() {
    ...
    this.valLongname.value = this.editor.world.longname || "Unnamed";
    ...
    this.valTilesetPath.value = this.editor.dataRaw.resource.find(res=>res.id=="map").src || "https://raw.githubusercontent.com/mroyale/assets/legacy/img/game/smb_map_new.png";
    this.valObjsheetPath.value = this.editor.dataRaw.resource.find(res=>res.id=="obj").src || "https://raw.githubusercontent.com/mroyale/assets/legacy/img/game/smb_obj.png";
    this.element.style.display = "block";
};
WorldTool.prototype.save = function() {
    ...
    try {
        ...
        this.editor.world.longname = this.valLongname.value;
        ...
        this.editor.display.resource.loadTexture({ "id":"map", "src":this.valTilesetPath.value });
        this.editor.display.resource.loadTexture({ "id":"obj", "src":this.valObjsheetPath.value });
        
        for(var i=0;i<this.editor.dataRaw.resource.length;i++) {
            var res = this.editor.dataRaw.resource[i];
            if(res.id == "map") { res.src = this.valTilesetPath.value; }
            if(res.id == "obj") { res.src = this.valObjsheetPath.value; }
        }
    } catch (_0x5286c3) {
...
function LevelTool(editor) {
    ...
    this.btnDelete = document.getElementById("editor-tool-level-del");
...
LevelTool.prototype.deleteZone = function() {
    var curZone = this.editor.currentZone;
    if (curZone.id == this.editor.world.getInitialZone().id) return alert("Can't delete the primary zone");
    if (!curZone.id) return alert("No zone selected?!");
    
    var i=0;
    for(; i<this.level.zones.length; ++i) {
        if (this.level.zones[i].id == curZone.id) break;
    }

    if (i == this.level.zones.length) return alert("Non-existing zone selected?!");
    if(!window.confirm("Really delete zone "+curZone.id+"?")) return;
    this.level.zones.splice(i,1);
    var info = this.editor.world.getLevel(this.editor.world.initial);
    this.editor.setZone(this.editor.world.getInitialZone(), info.id, info.initial);
    app.menu.list.generate()
};
...
ZoneTool.prototype.addLayer = function() {
    var z = parseInt(window.prompt("Z [less than 0 is background, greater is 1]"));
...
ZoneTool.prototype.save = function() {
    ...
    try {
        ...
        if (isNaN(id) || isNaN(x) || isNaN(y) || isNaN(c)) throw "input is not a number";
        this.zone.id = id;
        this.zone.initial = shor2.encode(x,y);
...
ObjectTool.prototype.input = function(lastInput, mouse, keys) {
    ...
    if (mouse.lmb) {
        ...
        if (mouse.mmb && !this.mmbx) {
            ...
            var objDefinition = objDefs[newObj.type];
            if(objDefinition && objDefinition.paramDefs) {
                if(newObj.param.length > objDefinition.paramDefs.length) { newObj.param.length = objDefinition.paramDefs.length; }
            } else {
                newObj.param.length = 0;
            }
...
ObjectTool.prototype.update = function() {
    try {
        ...
        var objDefinition = objDefs[this.obj.type];
        if(objDefinition && objDefinition.paramDefs) {
            if(this.obj.param.length > objDefinition.paramDefs.length) { this.obj.param.length = objDefinition.paramDefs.length; }
        } else {
            this.obj.param.length = 0;
        }
        this.updParamTools();
...
ReferenceTool.prototype.image = function() {
    var refImg = "ref" + parseInt(0x1000 * Math.random());
    this.editor.display.resource.load([{
        'id': refImg,
        'src': this.valImg.value
    }]);
    this.editor.reference = refImg;
};
...
/* Hey, isn't this just an outdated, obfuscated version of game.js? Can we take this out? */
"use strict";

function SettingsTool(_0x4b0f80) {
    this.editor = _0x4b0f80;
    this.element = document.getElementById("editor-tool-settings");
    var that = this;
    this.btnApply = document.getElementById("editor-tool-settings-apply");
    this.btnApply.onclick = function() {
        that.apply();
    };
    this.btnRevert = document.getElementById("editor-tool-settings-revert");
    this.btnRevert.onclick = function() {
        that.revert();
    };
    this.btnToggleTransparency = document.getElementById("editor-tool-settings-transparency");
    this.btnToggleTransparency.onclick = function() {
        that.toggleTransparency();
    };
    this.btnToggleGrid = document.getElementById("editor-tool-settings-grid");
    this.btnToggleGrid.onclick = function() {
        that.toggleGrid();
    };
}
SettingsTool.prototype.image = function() {};
SettingsTool.prototype.reload = function() {
    this.load();
};
SettingsTool.prototype.load = function() {
    this.element.style.display = "block";
};
SettingsTool.prototype.apply = function() {
    this.editor.isolateLayer = true;
};
SettingsTool.prototype.revert = function() {
    this.editor.isolateLayer = false;
};
SettingsTool.prototype.toggleGrid = function() {
    this.editor.drawGrid = !this.editor.drawGrid;
};
SettingsTool.prototype.toggleTransparency = function() {
    this.editor.disableLayerTransparency = !this.editor.disableLayerTransparency;
};
SettingsTool.prototype.save = function() {};
SettingsTool.prototype.destroy = function() {
    this.element.style.display = "none";
};
"use strict";
...
function Resource(resource) {
    ...
    this.texture.cache = {};
    this.texture.load = 0;
    this.loadTexture({ id: "editor_ui", src: "https://raw.githubusercontent.com/mroyale/assets/legacy/img/editor/editor_ui.png" });
    this.load(resource);
}
...
Resource.prototype.loadTexture = function (res, failSafe) {
    var texture = this.texture;
    var that = this;
    if (true) {
      var resource = new Image();
      resource.setAttribute('crossOrigin', '');
      resource.onload = function () {
        if((res.id == "map" || res.id == "obj") && !failSafe) {
            document.getElementById("editor-tool-world-load-failed").style.display = "none";
        }
        texture.cache[res.id] = resource;
        texture.load--;
      };
      resource.onerror = function () {
        texture.load--;
        if(res.id == "map") {
            that.loadTexture({ "id":"map", "src":"https://raw.githubusercontent.com/mroyale/assets/legacy/img/game/smb_map_new.png" }, true);//failsafe
            document.getElementById("editor-tool-world-load-failed").style.display = "";
            document.getElementById("failedToLoadWhat").innerText = "tileset";
        }
        if(res.id == "obj") {
            that.loadTexture({ "id":"obj", "src":"https://raw.githubusercontent.com/mroyale/assets/legacy/img/game/smb_obj.png" }, true);//failsafe
            document.getElementById("editor-tool-world-load-failed").style.display = "";
            document.getElementById("failedToLoadWhat").innerText = "object sheet";
        }
      };
      resource.src = res.src + '?v=2.1.0';
      texture.load++;
    }
};
...
Display.prototype.drawMap = function(data, depth, z) {
    ...
    for (i = 0x0; i < data.length; i++) {
        ...
        for (var j = screenLeft; j < screenRight; j++) {
            var checkerSprite = util.sprite.getSprite(uiTexture, 0x7);
            if(this.game.drawGrid && ((i % 2 === 0 && j % 2 === 0) || (i % 2 === 1 && j % 2 === 1))) {
                context.drawImage(uiTexture, checkerSprite[0x0], checkerSprite[0x1], Display.TEXRES, Display.TEXRES, Display.TEXRES * j, Display.TEXRES * (i), Display.TEXRES, Display.TEXRES);
            }
            ...
            if (drawAll || tile.depth === depth) {
                ...
                if (!(sprite[0x0] === 480 && sprite[0x1] === 0) || this.game.previewMode) {
                    ...
                    var rest = false;
                    if(z !== this.game.currentLayer.z && !this.game.disableLayerTransparency) { context.save(); rest = true; context.globalAlpha = 0.5; }

                    if (tileDefs[tile8.definition] && tileDefs[tile8.definition].name.includes("INVISIBLE")) {
                        context.save();
                        context.globalAlpha = 0.25;
                        var invisSprite = util.sprite.getSprite(mapTexture, 24);
                        context.drawImage(mapTexture, invisSprite[0x0], invisSprite[0x1], Display.TEXRES, Display.TEXRES, Display.TEXRES * j, Display.TEXRES * (i - t), Display.TEXRES, Display.TEXRES);
                        var invisObject = GameObject.OBJECT(parseInt(tile8.data));
                        if(invisObject && invisObject.SPRITE && invisObject.SPRITE[0]) {
                            context.globalAlpha = 0.5;
                            var objectSprite = invisObject.SPRITE[0].INDEX;
                            var invisObjectSprite = util.sprite.getSprite(objTexture, objectSprite);
                            context.drawImage(objTexture, invisObjectSprite[0x0], invisObjectSprite[0x1], Display.TEXRES, Display.TEXRES, Display.TEXRES * j, Display.TEXRES * (i - t), Display.TEXRES, Display.TEXRES);
                        }
                        context.restore();
                    }
                    

                    context.drawImage(mapTexture, sprite[0x0], sprite[0x1], Display.TEXRES, Display.TEXRES, Math.round(Display.TEXRES * j), Math.round(Display.TEXRES * (i - t)), Display.TEXRES, Display.TEXRES);
...
Display.prototype.drawObject = function() {
    for (var context = this.context, _0x59e855 = this.game.getZone(), _0x48dacd = _0x59e855.dimensions(), _0x19005f = this.canvas.width / Display.TEXRES * 0.75 / this.camera.scale, _0x4775a7 = Math.max(0x0, Math.min(_0x48dacd.x, parseInt(this.camera.pos.x - _0x19005f))), st = Math.max(0x0, Math.min(_0x48dacd.x, parseInt(this.camera.pos.x + _0x19005f))), _0x1a3274 = [], _0x19005f = [], _0x1d56bc = 0x0; _0x1d56bc < this.game.objects.length; _0x1d56bc++) {
        var rx = this.game.objects[_0x1d56bc];
        rx.level === _0x59e855.level && rx.zone === _0x59e855.id && rx.pid !== this.game.pid && rx.pos.x >= _0x4775a7 && rx.pos.x <= st && (rx.write && !this.game.disableText && rx.write(_0x19005f), rx.draw && rx.draw(_0x1a3274));
    ...
    for (_0x1d56bc = 0x0; _0x1d56bc < _0x1a3274.length; _0x1d56bc++) {
        var sprite = _0x1a3274[_0x1d56bc],
            st = util.sprite.getSprite(_0x4775a7, sprite.index),
            rx = !!sprite.reverse,
            ry = false,
            restore = false;
        switch (sprite.mode) {
            ...
            case 0x1:
                ...
                break;
            case 0x2:
                0x0 === parseInt(0.5 * this.game.frame) % 0x2 && (context.save(), restore = true, context.globalCompositeOperation = "lighter");
                break;
            case 0x3:
                ry = true;
                break;
            default:
                0xa0 <= sprite.mode && 0xc0 > sprite.mode && (context.save(), restore = true, context.globalAlpha = parseFloat(sprite.mode - 0xa0) / 0x20);
        }
        if (rx || ry) context.save(), context.scale(rx ? -0x1 : 0x1, ry ? -0x1 : 0x1);
        _0x59e855 = rx ? -0x1 * Display.TEXRES * sprite.pos.x - Display.TEXRES : Display.TEXRES * sprite.pos.x;
        sprite = ry ? -0x1 * Display.TEXRES * (_0x48dacd.y - sprite.pos.y - 0x1) - Display.TEXRES : Display.TEXRES * (_0x48dacd.y - sprite.pos.y - 0x1);
        context.drawImage(_0x4775a7, st[0x0], st[0x1], Display.TEXRES, Display.TEXRES, _0x59e855, sprite, Display.TEXRES, Display.TEXRES);
        (rx || ry) && context.restore();
        restore && context.restore();
    }
    for (_0x1d56bc = 0x0; _0x1d56bc < _0x19005f.length; _0x1d56bc++) _0x1a3274 = _0x19005f[_0x1d56bc], _0x59e855 = Display.TEXRES * _0x1a3274.pos.x + 0.5 * Display.TEXRES, sprite = Display.TEXRES * (_0x48dacd.y - _0x1a3274.pos.y - 0x1) + 0.5 * Display.TEXRES, context.fillStyle = _0x1a3274.color, context.font = _0x1a3274.size * Display.TEXRES + "px SmbWeb", context.textAlign = "center", context.fillText(_0x1a3274.text, _0x59e855, sprite);
};
...
EditorDisplay.prototype.draw = function() {
    ...
    if (this.resource.ready()) {
        ...
        var zone = this.game.getZone();
        for (var i=0; i<zone.layers.length; i++) {
            if(this.game.isolateLayer) {
                if(zone.layers[i].z != this.game.currentLayer.z) {
                    continue;
                }
            }
            this.drawMap(zone.layers[i].data, false, zone.layers[i].z);
            if (zone.layers[i].z == 0) {
                this.drawMap(zone.layers[i].data, true, zone.layers[i].z);
            }
        }
        this.drawBorder();
        this.drawCursor();
...
EditorDisplay.prototype.drawReference = function() {
    ...
    if (this.game.reference && this.game.showRef) {
        ...
        if (texture) {
            ...
            context.drawImage(texture, 0, 0, texture.width, texture.height, this.game.offsetRef.x, this.game.offsetRef.y, texture.width, texture.height);
            context.restore();
        }
    }
};
...
EditorDisplay.prototype.drawBorder = function() {
    var context = this.context,
        pos = vec2.scale(this.game.getZone().dimensions(), Display.TEXRES);
    context.lineWidth = 0x1;
    context.strokeStyle = "#FFFFFF";
    context.beginPath();
    context.moveTo(-(0.1 * Display.TEXRES), -(0.1 * Display.TEXRES));
    context.lineTo(pos.x + 0.1 * Display.TEXRES, -(0.1 * Display.TEXRES));
    context.lineTo(pos.x + 0.1 * Display.TEXRES, pos.y + 0.1 * Display.TEXRES);
    context.lineTo(-(0.1 * Display.TEXRES), pos.y + 0.1 * Display.TEXRES);
    context.lineTo(-(0.1 * Display.TEXRES), -(0.1 * Display.TEXRES));
    context.stroke();
};
EditorDisplay.prototype.drawCursor = function() {
    if (this.game.tool && void 0x0 !== this.game.tool.brush) {
        var context = this.context,
            dim = this.game.getZone().dimensions(),
            mouse = vec2.chop(this.camera.unproject(this.game.input.mouse.pos));
        0x0 > mouse.x || mouse.x >= dim.x || 0x0 > mouse.y || mouse.y >= dim.y || (context.fillStyle = "rgba(255,255,255,0.5)", context.fillRect(mouse.x * Display.TEXRES, mouse.y * Display.TEXRES, Display.TEXRES, Display.TEXRES));
    }
};
        ...
        for (var i = 0x0; i < zone.obj.length; i++) {
            ...
            if (obj.type === 0x25) {
                ...
                if (objType && objType.SPRITE && objType.SPRITE[0x0]) {
                    ...
                    context.drawImage(objTexture, sprite[0x0], sprite[0x1], Display.TEXRES, Display.TEXRES, Math.round(pos.x * Display.TEXRES), Math.round((height - pos.y - 0x1) * Display.TEXRES), Display.TEXRES, Display.TEXRES);
                }
            }
        }
    }
...
World.prototype.getLevel = function(_0x58a42b) {
    for (var i = 0x0; i < this.levels.length; i++) {
        ...
        if (level.id === _0x58a42b) return level;
    }
};
...
Level.prototype.getInitial = function() {
    for (var i = 0x0; i < this.zones.length; i++) {
        var level = this.zones[i];
        if (level.id === this.initial) return level;
    }
};
Level.prototype.getWarp = function(id) {
    for (var i = 0x0; i < this.zones.length; i++)
        for (var zone = this.zones[i], i = 0x0; i < zone.warp.length; i++) {
            var warp = zone.warp[i];
            if (warp.id === id) return {
                'level': this.id,
                'zone': zone.id,
                'pos': shor2.decode(warp.pos),
                'data': warp.data
...
function Editor(data) {
    ...
    this.drawGrid = true;
    this.disableLayerTransparency = false;
...
Editor.prototype.compile = function() {
    ...
    outData.longname = this.world.longname;
    ...
    if(this.dataRaw.group) { outData.group = this.dataRaw.group; }
...
Editor.prototype.setTool = function(toolType) {
    ...
    switch (toolType) {
        ...
        case "ref":
            ...
            break;
        case "settings":
            this.tool = new SettingsTool(this), this.tool.load();
            break;
    }
...
Editor.prototype.doInput = function() {
    ...
    this.inx71 = keys[0x47]; // G -> Toggle Reference (change to R if editor grid gets backported from Deluxe)
    if (keys[0x50] && !this.inx80 && document.activeElement === document.body) {
        this.previewMode = !this.previewMode;
        this.inx80 = true;
    }
    this.inx80 = keys[0x50]; // P -> Toggle Preview
};
